...
mod tests {
    ...
    fn go_to() {
        ...
        let mut string = String::new();
        let mut history = History::new();
        history.apply(&mut string, Add('a')).unwrap();
        history.apply(&mut string, Add('b')).unwrap();
        history.apply(&mut string, Add('c')).unwrap();
        history.apply(&mut string, Add('d')).unwrap();
        history.apply(&mut string, Add('e')).unwrap();
        assert_eq!(string, "abcde");
        history.undo(&mut string).unwrap();
        history.undo(&mut string).unwrap();
        assert_eq!(string, "abc");
        let abcde = history.branch();
        history.apply(&mut string, Add('f')).unwrap();
        history.apply(&mut string, Add('g')).unwrap();
        assert_eq!(string, "abcfg");
        history.undo(&mut string).unwrap();
        let abcfg = history.branch();
        history.apply(&mut string, Add('h')).unwrap();
        history.apply(&mut string, Add('i')).unwrap();
        history.apply(&mut string, Add('j')).unwrap();
        assert_eq!(string, "abcfhij");
        history.undo(&mut string).unwrap();
        let abcfhij = history.branch();
        history.apply(&mut string, Add('k')).unwrap();
        assert_eq!(string, "abcfhik");
        history.undo(&mut string).unwrap();
        let abcfhik = history.branch();
        history.apply(&mut string, Add('l')).unwrap();
        assert_eq!(string, "abcfhil");
        history.apply(&mut string, Add('m')).unwrap();
        assert_eq!(string, "abcfhilm");
        let abcfhilm = history.branch();
        history.go_to(&mut string, abcde, 2).unwrap().unwrap();
        history.apply(&mut string, Add('n')).unwrap();
        history.apply(&mut string, Add('o')).unwrap();
        assert_eq!(string, "abno");
        history.undo(&mut string).unwrap();
        let abno = history.branch();
        history.apply(&mut string, Add('p')).unwrap();
        history.apply(&mut string, Add('q')).unwrap();
        assert_eq!(string, "abnpq");

        let abnpq = history.branch();
        history.go_to(&mut string, abcde, 5).unwrap().unwrap();
        assert_eq!(string, "abcde");
        history.go_to(&mut string, abcfg, 5).unwrap().unwrap();
        assert_eq!(string, "abcfg");
        history.go_to(&mut string, abcfhij, 7).unwrap().unwrap();
        assert_eq!(string, "abcfhij");
        history.go_to(&mut string, abcfhik, 7).unwrap().unwrap();
        assert_eq!(string, "abcfhik");
        history.go_to(&mut string, abcfhilm, 8).unwrap().unwrap();
        assert_eq!(string, "abcfhilm");
        history.go_to(&mut string, abno, 4).unwrap().unwrap();
        assert_eq!(string, "abno");
        history.go_to(&mut string, abnpq, 5).unwrap().unwrap();
        assert_eq!(string, "abnpq");
    }
}
